(() => {
    ...
    function headerSearchToggle() {
        ...
        if (searchForm) searchFormBtn.addEventListener("click", (() => {
    ...
    function productCardOrder() {
        ...
        if (buttonsRect.length) buttonsRect.forEach((btn => {
            const productCard = btn.closest(".product-card");
            const order = productCard.querySelector(".product-card__order");
            btn.addEventListener("click", (() => {
                if (!order.classList.contains("_open")) handleOpen(); else handleClose();
            }));
            function handleOpen() {
    ...
    function productNumber() {
        ...
        if (numbers.length) numbers.forEach((n => {
            ...
            btnMinus.addEventListener("click", (() => value.value > 1 && value.value--));
        }));
    }
    mediaAdaptive();
    spoller();
